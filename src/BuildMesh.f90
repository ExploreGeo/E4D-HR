...
!__________________________________________________________________________________________________
    subroutine build_tetgen4(i_flag)
        ...
        open (22, file="mesh/"//trim(mesh_prefix)//".trn", status='unknown', position='append')
        ...
        open (33, file="mesh/surface.poly", status='replace', action='write')
        ...
        inquire (file='mesh/surface.1.node', exist=file_exists)
        if (file_exists) call system('rm mesh/surface.1.node');
        inquire (file='mesh/surface.1.ele', exist=file_exists)
        if (file_exists) call system('rm mesh/surface.1.ele');
        inquire (file='mesh/surface.1.neigh', exist=file_exists)
        if (file_exists) call system('rm mesh/surface.1.neigh');
        inquire (file='mesh/surface.1.edge', exist=file_exists)
        if (file_exists) call system('rm mesh/surface.1.edge');
        write (*, *) "Calling Triangle"
        call system(trim(tricom)//" -pnq30e mesh/surface.poly 2>&1 | tee triangle_output.log")
        ...
        call system("cp mesh/surface.1.ele mesh/surface.1.ele.old")
        open (77, file="mesh/surface.1.ele.old", status='old', action='read')
        open (88, file="mesh/surface.1.ele", status='replace', action='write')
        read (77, *) d1, d2, d3
        write (88, *) d1, 3, 1
        do i = 1, d1
...
!____________________________________________________________________________
        open (23, file='mesh/surface.1.node', status='old', action='read')
...
!!!!!!!!!!!!!!!!!!NOW WE ARE READY TO CONSTRUCT THE TETGEN INPUT FILE!!!!!!!!!!!!!!!!!!!!!!
        ...
        open (11, file="mesh/"//cfg_filename, status='replace', action='write')
        ...
        open (44, file="mesh/surface.1.ele", status='old', action='read')
...
1001    continue
        ...
        inquire (file="mesh/"//cfg_filename(1:npre)//'1.node', exist=file_exists)
        if (file_exists) call system('rm '//"mesh/"//cfg_filename(1:npre)//'1.node')
        inquire (file="mesh/"//cfg_filename(1:npre)//'1.ele', exist=file_exists)
        if (file_exists) call system('rm '//"mesh/"//cfg_filename(1:npre)//'1.ele')
        inquire (file="mesh/"//cfg_filename(1:npre)//'1.face', exist=file_exists)
        if (file_exists) call system('rm '//"mesh/"//cfg_filename(1:npre)//'1.face')
        inquire (file="mesh/"//cfg_filename(1:npre)//'1.neigh', exist=file_exists)
        if (file_exists) call system('rm '//"mesh/"//cfg_filename(1:npre)//'1.neigh')
        ...
        if (nz > 0) then
            write (*, *) trim(tetcom) // " -pnq" // trim(qual) // "a" // &
                trim(max_vol) // "aAAnnefV mesh/" // trim(cfg_filename)
            call system(trim(tetcom) // " -pnq" // trim(qual) // "a" // &
                trim(max_vol) // "aAAnnefV mesh/" // trim(cfg_filename) // " 2>&1 | tee tetgen_output.log")
        else
            write (*, *) trim(tetcom) // " -pnq" // trim(qual) // "a" // &
                trim(max_vol) // "AAnnefV mesh/" // trim(cfg_filename)
            call system(trim(tetcom) // " -pnq" // trim(qual) // "a" // &
                trim(max_vol) // "AAnnefV mesh/" // trim(cfg_filename) // " 2>&1 | tee tetgen_output.log")
        end if
        !call system('rm mesh/surface.*')
        ...
        if (vtk_flag .eq. 1) then
            inquire (file="mesh/"//trim(mesh_prefix)//".1.node", exist=file_exists)
            if (file_exists) then
                inquire (file="mesh/"//trim(mesh_prefix)//".1.ele", exist=file_exists)
                if (file_exists) then
                    inquire (file="mesh/"//trim(mesh_prefix)//".sigma", exist=file_exists)
                    if (file_exists) then
                        ! call export_mesh_model(trim(mesh_prefix), nz)
                    else
                        call mesh_error_log(106, "Cannot find "//"mesh/"//trim(mesh_prefix)//".sigma")
                    end if
                else
                    call mesh_error_log(102, "Cannot find "//"mesh/"//trim(mesh_prefix)//".1.ele")
                end if
            else
                call mesh_error_log(101, "Cannot find "//"mesh/"//trim(mesh_prefix)//".1.node")
            end if
        end if
...
!___________________________________________________________________________________________________
    subroutine check_nodes(mpre)
        ...
        open (20, file='mesh/'//mpre(1:npre)//'trn', status='old', action='read')
        ...
        !read the nodes
        open (20, file='mesh/'//mpre(1:npre)//'1.node', status='old', action='read')
        read (20, *) nnods
...
100             continue
        ...
        call system("cp mesh/"//mpre(1:npre)//"1.node mesh/"//trim(mpre)//".1_orig.node")
        open (20, file='mesh/'//mpre(1:npre)//'1.node', status='replace', action='write')
...
!___________________________________________________________________________________________________
    subroutine check_minp(indx, ios)
        ...
        select case (indx)
          ...
          case (24)
            inquire (file='mesh/surface.1.node', exist=exst)
            if (.not. exst) goto 10
            inquire (file='mesh/surface.1.ele', exist=exst)
            if (.not. exst) goto 10
            inquire (file='mesh/surface.1.neigh', exist=exst)
            if (.not. exst) goto 10
            inquire (file='mesh/surface.1.edge', exist=exst)
            if (.not. exst) goto 10
10          continue
            if (.not. exst) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine build_sig(cfg_filename, npre, nzon, zsigs, zsigsi, zlabs, ifg)
        ...
        open (11, file='mesh/'//cfg_filename(1:npre)//"1.ele", status='old', action='read')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine build_sig_triangle
        ...
        integer :: nsigs, i
        real*8 :: sigval
        open (66, file='mesh/surface.sigma', status='replace', action='write')
        open (55, file='mesh/surface.1.ele', status='old', action='read')
        read (55, *) nsigs
        close(55)
        sigval = 0.1
        write (66, *) nsigs
